...
from history_guard import append_board_history
from production_backtest import build_walk_forward_scores
from status_guard import build_status

AFTER_CLOSE_TIME = dt_time(19, 0)


def _target_trade_date(uds, now: datetime, calendar: pd.Series) -> tuple[pd.Timestamp | None, str]:
...
def _recover_previous_recommendation(uds, target_date: pd.Timestamp, history_file: Path) -> pd.DataFrame:
...
def install(uds) -> None:
    uds._target_trade_date = lambda now, calendar: _target_trade_date(uds, now, calendar)
    uds._status = lambda target, status, reason, latest_source_dates=None: build_status(
        uds._read_json, uds.LATEST_FILE, target, status, reason, latest_source_dates
    )
    uds._build_walk_forward_scores = build_walk_forward_scores
    ...
    def _load_recommendation_holding(target_date: pd.Timestamp, history_file: Path) -> pd.DataFrame:
        ...
        if isinstance(state, dict) and state:
            ...
            if state.get("as_of_trade_date"):
                return pd.DataFrame()
        return _recover_previous_recommendation(uds, target_date, history_file)
